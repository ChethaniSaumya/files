...
async function executeDirectBondingCurveBuy(params) {
    ...
    try {
        ...
        const walletBalance = await connection.getBalance(buyer);
        ...
        if (walletBalance < totalTransactionCost) {
            ...
            throw new Error(
                ...
                `Breakdown:\n` +
                `  • Max Buy: ${(maxSolCost / LAMPORTS_PER_SOL).toFixed(6)} SOL\n` +
                `  • Priority Fee: ${(priorityFeeLamports / LAMPORTS_PER_SOL).toFixed(6)} SOL\n` +
                `  • ATA Creation: ${(ataCreationCost / LAMPORTS_PER_SOL).toFixed(6)} SOL\n` +
                `  • Network Fee: ${(networkBaseFee / LAMPORTS_PER_SOL).toFixed(6)} SOL`
            );
        }
