...
// ========== INSTANT PARALLEL TOKEN PROCESSING WITH DEDUPLICATION ==========
async function processTokenInstantly(tokenData, platform) {
    ...
    if (!tokenDeduplication.shouldProcessToken(tokenAddress, safePlatform)) {
        console.log(`⏭️ DEDUPLICATION: Skipping duplicate token ${tokenAddress.substring(0, 16)}...`);
        return;
    }
    ...
    if (activeThreadCount >= MAX_CONCURRENT_THREADS) {
        ...
        // Remove from deduplication since we're not processing it
        tokenDeduplication.processedTokens.delete(tokenDeduplication.getTokenKey(tokenAddress, safePlatform));
        return;
    }
    ...
    (async () => {
        try {
            ...
            // On error, you might want to remove from deduplication to allow retry
            // tokenDeduplication.processedTokens.delete(tokenDeduplication.getTokenKey(tokenAddress, safePlatform));
        } finally {
            activeThreadCount--;
...
// ========== COMPREHENSIVE TOKEN DEDUPLICATION SYSTEM ==========
class TokenDeduplication {
    constructor() {
        this.processedTokens = new Map(); // tokenAddress -> timestamp
        this.DEDUPLICATION_WINDOW = 30000; // 30 seconds
        this.cleanupInterval = null;

        this.startCleanupInterval();
    }

    // Check if token should be processed
    shouldProcessToken(tokenAddress, platform = 'unknown') {
        const now = Date.now();
        const key = this.getTokenKey(tokenAddress, platform);

        if (this.processedTokens.has(key)) {
            const lastProcessed = this.processedTokens.get(key);
            const timeSinceLastProcess = now - lastProcessed;

            if (timeSinceLastProcess < this.DEDUPLICATION_WINDOW) {
                console.log(`⏭️ DEDUPLICATION: Token ${tokenAddress.substring(0, 16)}... processed ${timeSinceLastProcess}ms ago, skipping`);
                return false;
            }
        }

        // Mark as processed
        this.processedTokens.set(key, now);
        console.log(`✅ DEDUPLICATION: Token ${tokenAddress.substring(0, 16)}... marked for processing`);
        return true;
    }

    // Create unique key for token (address + platform to handle same token on different platforms)
    getTokenKey(tokenAddress, platform) {
        return `${tokenAddress}-${platform}`;
    }

    // Force mark token as processed (useful for manual overrides)
    markTokenAsProcessed(tokenAddress, platform = 'unknown') {
        const key = this.getTokenKey(tokenAddress, platform);
        this.processedTokens.set(key, Date.now());
        console.log(`🔒 DEDUPLICATION: Token ${tokenAddress.substring(0, 16)}... manually marked as processed`);
    }
    ...
    cleanup() {
        ...
        for (const [key, timestamp] of this.processedTokens.entries()) {
        ...
        if (cleanedCount > 0) {
            console.log(`🧹 DEDUPLICATION: Cleaned ${cleanedCount} old entries`);
        }
    }
    ...
    getStats() {
        return {
            activeTokens: this.processedTokens.size,
    ...
    clear() {
        const count = this.processedTokens.size;
        this.processedTokens.clear();
        console.log(`🗑️ DEDUPLICATION: Cleared all ${count} entries`);
        return count;
    }
}

// Create global instance
const tokenDeduplication = new TokenDeduplication();
    ...
    constructor() {
        ...
        this.settings = {
            ...
            globalSnipeSettings: {
                ...
                priorityFee: 0.0008  // ✅ ENSURE THIS HAS A DEFAULT VALUE
            }
        };
...
class EnhancedBotState extends BotState {
    ...
    // In EnhancedBotState class - FIX THE addToList method
...
async function getBondingCurveData(connection, bondingCurve) {
    try {
        ...
        // ✅ CORRECT OFFSETS for Pump.fun bonding curve
        ...
        // ✅ CRITICAL FIX: Creator is at offset 49 (8 + 8*5 + 1 = 49)
...
function createPumpFunBuyInstruction(
    ...
    // ✅ CRITICAL FIX: Use correct PDA seeds for creator vault
    // The creator vault is derived from: ["creator-vault", creator_pubkey]
    console.log(`🔑 Calculating creator vault PDA with creator: ${creator.toBase58()}`);
    ...
    console.log(`🦺 Creator Vault PDA: ${creatorVault.toBase58()} (bump: ${creatorVaultBump})`);
...
async function executeDirectBondingCurveBuy(params) {
    ...
    console.log('\n' + '🎯'.repeat(40));
    console.log('ULTRA-FAST BONDING CURVE BUY - NO RPC FOR RESERVES');
    ...
    try {
        ...
        // ✅ STEP 1: USE PRE-EXTRACTED BONDING CURVE STATE - NO RPC!
        let bondingCurveState = params.bondingCurveState;
        let creator = new PublicKey(params.creatorWallet);

        if (!bondingCurveState) {
            console.log('⚠️ No pre-extracted bonding curve state, falling back to RPC...');
            // Fallback to RPC if no pre-extracted state
            ...
            console.log('✅ Using pre-extracted bonding curve state - NO RPC needed!');
            console.log(`💰 Reserves: ${bondingCurveState.virtualSolReserves} SOL, ${bondingCurveState.virtualTokenReserves} tokens`);
        ...
        // ✅ FIX: Convert priority fee to lamports correctly
        ...
        // Quick wallet balance check (optional but recommended)
        ...
        console.log(`   Using Creator: ${creator.toBase58()}`);
        ...
        // ✅ STEP 6: ONLY 2 RPC CALLS REQUIRED NOW
        console.log('\n⏳ Getting fresh blockhash (1 of 2 required RPC calls)...');
        ...
        console.log('\n📤 Sending transaction to Solana network (2 of 2 required RPC calls)...');
        ...
        console.log('\n' + '='.repeat(80));
        console.log('✅ ULTRA-FAST BUY COMPLETE');
        console.log('='.repeat(80));
        ...
        console.log(`🔧 RPC Calls: 2 (blockhash + transaction submission)`);
        console.log('='.repeat(80) + '\n');
        ...
        const executionTime = Date.now() - startTime;
        console.error('\n' + '='.repeat(80));
        console.error('❌ ULTRA-FAST BUY FAILED');
...
async function snipeToken(tokenAddress, config) {
    ...
    console.log(`   config.amount: ${config.amount} SOL`);
    console.log(`   config.fees: ${config.fees}%`);
    console.log(`   config.priorityFee: ${config.priorityFee} SOL`);
    console.log('🔥'.repeat(40));

    // ✅ VALIDATE CONFIGURATION
    if (!config.amount || typeof config.amount !== 'number' || config.amount <= 0) {
        const error = new Error(`Invalid snipe amount: ${config.amount}`);
        broadcastToClients({
            type: 'snipe_error',
            data: {
                tokenAddress,
    ...
    if (!config.priorityFee || typeof config.priorityFee !== 'number' || config.priorityFee < 0) {
        console.log('⚠️ Priority Fee invalid, using default: 0.0005 SOL');
        config.priorityFee = 0.0005;
    }

    try {
        // ✅ EARLY BASIC BALANCE CHECK (keeps existing behavior)
        console.log('🏁 Checking wallet balance...');
        const hasBalance = await checkWalletBalance(connection);
        if (!hasBalance) {
        ...
        // ✅ FIX: This is where the DETAILED balance check happens
        // If it throws an error, we'll catch it below
        ...
        // ✅ ONLY BROADCAST SUCCESS IF WE ACTUALLY GOT HERE
        broadcastToClients({
            type: 'snipe_success',
            data: {
                tokenAddress,
        ...
        // ✅ FIX: Parse insufficient funds error for better frontend display
        const errorData = {
            tokenAddress,
            error: error.message,
            timestamp: new Date().toISOString()
        ...
        if (error.message.includes('INSUFFICIENT FUNDS')) {
            const needMatch = error.message.match(/Need:\s*([\d.]+)\s*SOL/);
            const haveMatch = error.message.match(/Have:\s*([\d.]+)\s*SOL/);
            const shortfallMatch = error.message.match(/Shortfall:\s*([\d.]+)\s*SOL/);

            errorData.errorType = 'insufficient_funds';
            errorData.details = {
                needed: needMatch ? parseFloat(needMatch[1]) : null,
                available: haveMatch ? parseFloat(haveMatch[1]) : null,
                shortfall: shortfallMatch ? parseFloat(shortfallMatch[1]) : null
            };

            errorData.formattedMessage = `💰 Insufficient Funds\nNeed: ${errorData.details.needed} SOL\nHave: ${errorData.details.available} SOL\nShortfall: ${errorData.details.shortfall} SOL`;
        } else {
            errorData.errorType = 'general_error';
            errorData.formattedMessage = `Snipe failed: ${error.message}`;
        }

        // ✅ NOW BROADCAST THE ERROR (not success!)
        broadcastToClients({
            type: 'snipe_error',
            data: errorData
        });
...
async function processTokenInstantSnipe(tokenData, platform) {
    ...
    // ✅ CHECK IF WE SHOULD ACTUALLY SNIPE
    console.log('Snipe All Mode - Detection Only:', botState.settings.detectionOnlyMode);

    if (!botState.settings.detectionOnlyMode) {
        console.log('⚡ EXECUTING INSTANT SNIPE (0ms delay)...');
        console.log('Config:', JSON.stringify(botState.settings.globalSnipeSettings, null, 2));
        ...
        try {
            ...
            if (snipeResult.success) {
                ...
                console.log(`Signature: ${snipeResult.signature}`);
            } else {
                console.error(`❌ SNIPE FAILED: ${snipeResult.error}`);
            }
        } catch (error) {
            console.error(`❌ SNIPE EXCEPTION:`, error.message);
            console.error('Stack:', error.stack);
        }
    } else {
        console.log('🛡️ DETECTION ONLY MODE - Snipe skipped');
    }
...
async function processNewToken(tokenData, platform) {
    // ✅ ADD WALLET MATCHING RIGHT HERE - AT THE VERY START
    console.log(`⚡ TOKEN RECEIVED: ${tokenData.mint} | Complete: ${tokenData.complete}`);

    // ✅ ULTRA-FAST PATH: Wallet matching check (0ms metadata delay)
    if (botState.settings.enableAdminFilter && tokenData.creator && tokenData.creator !== 'Unknown') {
        const walletResult = await checkWalletMatchInstantly(tokenData);

        if (walletResult.match) {
            console.log(`🎯 INSTANT WALLET MATCH in ${walletResult.matchTime}ms`);
            await handleInstantMatch(tokenData, walletResult);
            return; // Done in 50-100ms!
        }
    }

    // ... rest of your existing processNewToken code continues ...
    ...
    const bondingCurve = tokenData.bondingCurveAddress;
    const masterStartTime = tokenData.masterStartTime || Date.now();

    //...................................................

    console.log(`⚡ TOKEN RECEIVED: ${tokenAddress} | Complete: ${tokenData.complete}`);

    // ✅ ULTRA-FAST PATH: Wallet matching check (0ms metadata delay)
    if (botState.settings.enableAdminFilter && tokenData.creator) {
        const walletResult = await checkWalletMatchInstantly(tokenData);

        if (walletResult.match) {
            console.log(`🎯 INSTANT WALLET MATCH in ${Date.now() - masterStartTime}ms`);
            await handleInstantMatch(tokenData, walletResult);
            return; // Done in 50-100ms!
        }
    }

    // ✅ SNIPE ALL MODE: No metadata needed
    if (botState.settings.snipeAllTokens && !botState.settings.detectionOnlyMode) {
        console.log(`⚡ SNIPE ALL: Immediate execution`);
        await snipeToken(tokenAddress, botState.settings.globalSnipeSettings);
        return; // Done in 100-200ms!
    }

    // ✅ ONLY NOW: Get metadata for admin matching (slow path)
    if (!tokenData.complete || !tokenData.metadataAlreadyFetched) {
        console.log(`🔄 Slow path: Fetching metadata for admin matching...`);
        tokenData = await enhanceTokenData(tokenData);
    }
        ...
        if (botState.settings.enableAdminFilter) {
                    ...
                    if (primaryAdminConfig) {
                        ...
                        const detectedTokenData = {
                            ...
                            config: {
                                ...
                                priorityFee: (primaryAdminConfig.priorityFee !== undefined &&
                                    primaryAdminConfig.priorityFee !== null &&
                                    !isNaN(primaryAdminConfig.priorityFee) &&
                                    primaryAdminConfig.priorityFee >= 0)
                                    ? primaryAdminConfig.priorityFee
                                    : botState.settings.globalSnipeSettings.priorityFee,
                                mevProtection: primaryAdminConfig.mevProtection !== undefined
                                    ? primaryAdminConfig.mevProtection
                                    : botState.settings.globalSnipeSettings.mevProtection,
                                soundNotification: primaryAdminConfig.soundNotification || botState.settings.globalSnipeSettings.soundNotification
                            }
                        };

                        // ✅ Store token BEFORE any logging or snipe
                        botState.addDetectedToken(tokenAddress, detectedTokenData);

                        console.log('\n' + '🔴'.repeat(50));
                        console.log('🔴 PRIMARY ADMIN MATCH DETECTED');
                        console.log('🔴'.repeat(50));
                        console.log('Token:', tokenAddress);
                        console.log('Name:', detectedTokenData.name);
                        console.log('Symbol:', detectedTokenData.symbol);
                        console.log('Admin:', twitterData.handle);
                        console.log('Detection Only Mode:', botState.settings.detectionOnlyMode);
                        console.log('🔴'.repeat(50) + '\n');
                        ...
                        redLogger.matchFound(
                            ...
                            {
                                name: detectedTokenData.name, // ✅ NOW AVAILABLE
                                symbol: detectedTokenData.symbol, // ✅ NOW AVAILABLE
                                address: tokenAddress,
                                platform: platform
                            }
                        );
                        ...
                        // ✅ FIX: Broadcast with complete token data
                        const processingEndTime = Date.now();
                        console.log(`PROCESSING COMPLETE: ${processingEndTime - masterStartTime}ms total`);

                        broadcastToClients({
                            type: 'token_detected',
                            data: detectedTokenData // ✅ NOW HAS NAME/SYMBOL
                        });

                        logTokenProcessingDecision(tokenAddress, 'detected', 'Primary Twitter admin match', {
                            admin: twitterData.handle,
                            platform,
                            name: detectedTokenData.name, // ✅ NOW LOGGED
                            symbol: detectedTokenData.symbol // ✅ NOW LOGGED
                        });

                        // ✅ FIX: Use the COMPLETE config, not just primaryAdminConfig
                        if (!botState.settings.detectionOnlyMode) {
                            timingTracker.checkpoint(tokenAddress, 'snipeStart');
                            const snipeResult = await snipeToken(tokenAddress, detectedTokenData.config); // ✅ FIXED
                            timingTracker.checkpoint(tokenAddress, 'snipeEnd');

                            // ✅ Log primary admin match AFTER successful snipe
                            if (snipeResult.success) {
                                logPrimaryAdminMatch(detectedTokenData, snipeResult.signature, detectedTokenData.config);
                            }
            ...
            if (twitterData.type === 'community' && twitterData.id) {
                ...
                if (botState.settings.enableCommunityReuse) {
                    ...
                    if (communityUsed) {
                        ...
                        logTokenProcessingDecision(tokenAddress, 'rejected', 'Community already used', {
                            ...
                            platform,
                            name: completeTokenData.name // ✅ NOW AVAILABLE
                        });
                ...
                if (communityMatchResult) {
                    ...
                    // ✅ FIX: Create complete token data with all metadata
                    const detectedTokenData = {
                        ...completeTokenData,
                    ...
                    // ✅ FIX: Store token BEFORE any actions
                    botState.addDetectedToken(tokenAddress, detectedTokenData);
                    ...
                    // ✅ FIX: Broadcast with complete data
                    broadcastToClients({
                        type: 'token_detected',
                        data: detectedTokenData // ✅ NOW HAS NAME/SYMBOL
                    });
                    ...
                    if (communityMatchResult && communityMatchResult.matchType === 'primary_admin') {
                        ...
                        console.log('\n' + '🔴'.repeat(50));
                        console.log('🔴 PRIMARY COMMUNITY ADMIN MATCH');
                        console.log('🔴'.repeat(50));
                        console.log('Token:', tokenAddress);
                        console.log('Name:', detectedTokenData.name); // ✅ NOW AVAILABLE
                        console.log('Symbol:', detectedTokenData.symbol); // ✅ NOW AVAILABLE
                        console.log('Admin:', communityMatchResult.matchedEntity);
                        console.log('Community:', twitterData.id);
                        console.log('🔴'.repeat(50) + '\n');
                        ...
                        if (!botState.settings.detectionOnlyMode) {
                            ...
                            console.log('🔥'.repeat(50));
                            console.log(`Token: ${tokenAddress}`);
                            console.log(`Name: ${detectedTokenData.name}`); // ✅ NOW LOGGED
                            console.log(`Symbol: ${detectedTokenData.symbol}`); // ✅ NOW LOGGED
                            console.log(`Admin: ${communityMatchResult.matchedEntity}`);
                            ...
                            try {
                                timingTracker.checkpoint(tokenAddress, 'snipeStart');
                                const snipeResult = await snipeToken(tokenAddress, detectedTokenData.config);
                                timingTracker.checkpoint(tokenAddress, 'snipeEnd');

                                // ✅ FIX: Log primary admin match with complete data
                                if (snipeResult.success) {
                        ...
                        broadcastToClients({
                            ...
                            data: {
                                tokenData: {
                                    ...detectedTokenData,
                                    platform: platform || detectedTokenData.platform || 'unknown'
                                },
                                globalSnipeSettings: botState.settings.globalSnipeSettings,
                                timestamp: new Date().toISOString()
                            }
...
// Add this to your timing tracker or create a separate logging function
function logTokenProcessingDecision(tokenAddress, decision, reason, data = {}) {
    const logEntry = {
        ...
        reason,
        deduplication: {
            activeTokens: tokenDeduplication.processedTokens.size,
            isDuplicate: !tokenDeduplication.shouldProcessToken(tokenAddress, data.platform || 'unknown')
        },
        ...data
    };
    ...
    console.log(`Deduplication: ${logEntry.deduplication.isDuplicate ? 'DUPLICATE' : 'NEW'}`);
...
app.post('/api/global-snipe-settings', async (req, res) => {
    ...
    console.log(`   Received priorityFee: ${priorityFee} (type: ${typeof priorityFee})`);
    console.log('🔧'.repeat(40) + '\n');
    ...
    if (typeof priorityFee !== 'undefined' && priorityFee !== null) {
        const numericPriorityFee = Number(priorityFee);
        if (!isNaN(numericPriorityFee) && numericPriorityFee >= 0) {
            console.log(`✅ Updating priorityFee from ${botState.settings.globalSnipeSettings.priorityFee} to ${numericPriorityFee}`);
            botState.settings.globalSnipeSettings.priorityFee = numericPriorityFee;
        } else {
            console.log(`⚠️ Invalid priorityFee value: ${priorityFee}, keeping current value: ${botState.settings.globalSnipeSettings.priorityFee}`);
        }
    }
...
app.post('/api/update-existing-admins-amounts', async (req, res) => {
    try {
        const { amount, fees, priorityFee } = req.body;  // ✅ ADD priorityFee

        console.log(`🔄 Updating all existing admin amounts to ${amount} SOL, fees to ${fees}%, and priority fee to ${priorityFee} SOL`);
        ...
        for (const [id, config] of botState.primaryAdminList.entries()) {
            ...
            if (fees !== undefined) config.fees = fees;
            if (priorityFee !== undefined) config.priorityFee = priorityFee;  // ✅ ADD THIS LINE
            await saveAdminListToFirebase('primary_admins', config);
            primaryUpdated++;
        }
...
app.get('/api/debug/snipe-settings', (req, res) => {
    const settings = {
        isRunning: botState.isRunning,
        detectionOnlyMode: botState.settings.detectionOnlyMode,
        snipeAllTokens: botState.settings.snipeAllTokens,
        ...
        enablePrimaryDetection: botState.settings.enablePrimaryDetection,
        enableSecondaryDetection: botState.settings.enableSecondaryDetection,
        privateKeySet: !!botState.settings.privateKey,
        globalSnipeSettings: botState.settings.globalSnipeSettings,
        primaryAdminCount: botState.primaryAdminList.size,
        secondaryAdminCount: botState.secondaryAdminList.size
    };

    console.log('\n' + '📋'.repeat(50));
    console.log('CURRENT SNIPE SETTINGS DEBUG:');
    console.log('📋'.repeat(50));
    ...
    res.json({
        ...
        settings,
        explanation: {
            willExecuteSnipes: botState.isRunning && !botState.settings.detectionOnlyMode && botState.settings.privateKey,
            blockers: [
                ...(!botState.isRunning ? ['Bot is not running'] : []),
                ...(botState.settings.detectionOnlyMode ? ['Detection Only Mode is ON'] : []),
                ...(!botState.settings.privateKey ? ['Private key not set'] : [])
            ]
        }
    });
});

app.post('/api/debug/test-snipe-flow/:tokenAddress', async (req, res) => {
    try {
        const { tokenAddress } = req.params;

        console.log('\n' + '🧪'.repeat(50));
        console.log('🧪 TESTING SNIPE FLOW FOR TOKEN:', tokenAddress);
        console.log('🧪'.repeat(50));
        ...
        if (!detectedToken) {
            return res.status(404).json({
                success: false,
                error: 'Token not found in detected tokens',
                hint: 'Token must be detected first before testing snipe'
            });
        }

        console.log('Token found:', JSON.stringify(detectedToken, null, 2));

        // Check settings
        const canSnipe = botState.isRunning &&
            !botState.settings.detectionOnlyMode &&
            botState.settings.privateKey;

        console.log('Can snipe:', canSnipe);
        console.log('Bot running:', botState.isRunning);
        console.log('Detection only mode:', botState.settings.detectionOnlyMode);
        console.log('Private key set:', !!botState.settings.privateKey);

        if (!canSnipe) {
            return res.json({
                success: false,
                message: 'Snipe conditions not met',
                blockers: {
                    botNotRunning: !botState.isRunning,
                    detectionOnlyMode: botState.settings.detectionOnlyMode,
                    noPrivateKey: !botState.settings.privateKey
                }
            });
        }

        // Try to execute snipe
        console.log('Attempting to call snipeToken...');

        const config = detectedToken.config || botState.settings.globalSnipeSettings;
        console.log('Using config:', JSON.stringify(config, null, 2));

        const result = await snipeToken(tokenAddress, config);

        console.log('Snipe result:', JSON.stringify(result, null, 2));
        console.log('🧪'.repeat(50) + '\n');

        res.json({
            success: true,
            result,
            message: 'Snipe test completed'
        });

    } catch (error) {
        console.error('Test snipe error:', error);
        res.status(500).json({
            success: false,
            error: error.message,
            stack: error.stack
...
app.get('/api/deduplication-stats', (req, res) => {
    const stats = tokenDeduplication.getStats();
    res.json({
        success: true,
        stats,
        activeTokens: Array.from(tokenDeduplication.processedTokens.entries()).map(([key, timestamp]) => ({
            key,
...
app.post('/api/deduplication/clear-token', (req, res) => {
    const { tokenAddress, platform = 'unknown' } = req.body;

    if (!tokenAddress) {
        return res.status(400).json({ error: 'Token address is required' });
    }

    const key = tokenDeduplication.getTokenKey(tokenAddress, platform);
    const existed = tokenDeduplication.processedTokens.delete(key);

    res.json({
        success: true,
        cleared: existed,
        message: existed ?
            `Token ${tokenAddress.substring(0, 16)}... removed from deduplication` :
            `Token ${tokenAddress.substring(0, 16)}... not found in deduplication`
    });
});

app.post('/api/deduplication/clear-all', (req, res) => {
    const clearedCount = tokenDeduplication.clear();

    res.json({
        success: true,
        clearedCount,
        message: `Cleared ${clearedCount} tokens from deduplication`
    });
});

app.post('/api/deduplication/manual-process', (req, res) => {
    const { tokenAddress, platform = 'unknown' } = req.body;
    ...
    // Force allow processing by removing from deduplication
    const key = tokenDeduplication.getTokenKey(tokenAddress, platform);
    tokenDeduplication.processedTokens.delete(key);

    res.json({
        success: true,
        message: `Token ${tokenAddress.substring(0, 16)}... can now be processed again`
    });
});



// ========== WALLET MATCHING FUNCTION (PUT THIS AT TOP LEVEL) ==========
async function checkWalletMatchInstantly(tokenData) {
    const startTime = Date.now();
    ...
    console.log(`🔍 Checking wallet match for: ${tokenData.creator}`);

    // Check primary admin list
    const primaryMatch = botState.checkAdminInPrimary(tokenData.creator);
    if (primaryMatch) {
        console.log(`🎯 PRIMARY WALLET MATCH: ${tokenData.creator}`);
        return {
            match: true,
            type: 'primary',
            config: primaryMatch,
    ...
    // Check secondary admin list
    if (botState.settings.enableSecondaryDetection) {
        const secondaryMatch = botState.checkAdminInSecondary(tokenData.creator);
        if (secondaryMatch) {
            console.log(`🎯 SECONDARY WALLET MATCH: ${tokenData.creator}`);
            return {
                match: true,
                type: 'secondary',
                config: secondaryMatch,
    ...
    console.log(`❌ No wallet match for: ${tokenData.creator}`);
    return { match: false, matchTime: Date.now() - startTime };
}


// ✅ ADD THIS FUNCTION RIGHT AFTER checkWalletMatchInstantly
async function handleInstantMatch(tokenData, walletResult) {
    const tokenAddress = tokenData.mint;
    const masterStartTime = tokenData.masterStartTime || Date.now();
...
// ========== TOKEN DATA ENHANCEMENT FUNCTION ==========
async function enhanceTokenData(tokenData) {
    console.log(`🔄 Enhancing token data for: ${tokenData.mint}`);

    try {
        // Use the existing metadata fetching logic
        const enhancedData = await fetchTokenMetadata(tokenData);
        const twitterDataResult = await getTwitterDataFromToken(tokenData);

        return {
            ...tokenData,
            name: enhancedData.name || tokenData.name,
            symbol: enhancedData.symbol || tokenData.symbol,
            description: enhancedData.description,
            imageUrl: enhancedData.imageUrl,
            twitter: enhancedData.twitterUrl,
            twitterHandle: twitterDataResult.handle,
            twitterCommunityId: twitterDataResult.id,
            website: enhancedData.website,
            hasEnhancedData: enhancedData.hasEnhancedData,
